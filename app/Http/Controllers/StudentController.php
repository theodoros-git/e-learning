...
class StudentController extends Controller
{
    ...
    public function login_form(Request $request) {
        ...
        if (Auth::attempt(['username' => $request->username, 'password' => $request->password, 'is_student' => 1], $remember = $request->remember)) {
            ...
            return redirect('/students/dashboard')->withWelcome('Bienvenue. Content d\'être parmi nous ');;
    ...
    public function activity_files(int $id) {
        ...
        if (Auth::check()) {
            ...
            if (Auth()->user()->is_student == True) {
                ...
                if ($student->abonnement_is_active == true and $student->abonnement_date_end != now()->toDateTimeString() ) {
                    ...
                    $activity = Activite::find($id)->files;
                    $compte = 1;
                    
                    return view('dashboard.students.activity_files', [
                        'activity' => $activity,
                        'compte' => $compte,
                    ]);
                }
    ...
    public function download(Request $request, string $url) {
        ...
        if (Auth::check()) {
            ...
            if (Auth()->user()->is_student == True) {
                ...
                if ($student->abonnement_is_active == true and $student->abonnement_date_end != now()->toDateTimeString() ) {
                    ...
                    return response()->download(public_path('uploads/activity_files'.'/'.$url));
                }
